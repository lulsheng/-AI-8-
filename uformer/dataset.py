...
import paddle
from utils import is_png_file, is_image_file
from utils import load_img
from utils import Augment_RGB_paddle
...
class DataLoaderTrain(Dataset):
    ...
    def __init__(self, rgb_dir, img_options=None, target_transform=None):
        ...
        self.clean_filenames = [os.path.join(rgb_dir, gt_dir, x) for x in
            clean_files if is_image_file(x)]
        self.noisy_filenames = [os.path.join(rgb_dir, input_dir, x) for x in
            noisy_files if is_image_file(x)]
    ...
    def __getitem__(self, index):
        ...
        clean = paddle.to_tensor(np.float32(load_img(self.clean_filenames[tar_index])))
        noisy = paddle.to_tensor(np.float32(load_img(self.noisy_filenames[tar_index])))
...
class DataLoaderTrain_Gaussian(Dataset):
    ...
    def __init__(self, rgb_dir, noiselevel=5, img_options=None,
        target_transform=None):
        super(DataLoaderTrain_Gaussian, self).__init__()
        self.target_transform = target_transform
        clean_files = sorted(os.listdir(rgb_dir))
        self.clean_filenames = [os.path.join(rgb_dir, x) for x in
            clean_files if is_png_file(x)]
    ...
    def __getitem__(self, index):
        ...
        noisy = clean + np.float32(np.random.normal(0, self.noiselevel, np.
            array(clean).shape) / 255.0)
...
class DataLoaderTestA(Dataset):
    def __init__(self, rgb_dir, target_transform=None):
        super(DataLoaderTestA, self).__init__()

        self.target_transform = target_transform

        input_dir = 'demoire_testB_1664'
...
class DataLoaderTestSR(Dataset):
    ...
    def __getitem__(self, index):
        tar_index = index % self.tar_size
        LR = paddle.to_tensor(np.float32(load_img(self.LR_filenames[tar_index])))
